...
class Attention(nn.Module):
    def __init__(self, dim, head_dim, grid_atten, grid_size=1, drop=0.):
        super().__init__()
...
class HAT_NetBlock(nn.Module):
    def __init__(self, dim, num_heads, grid_atten, grid_size=1, mlp_ratio=2,
                 drop=0., drop_path=0., act_layer=nn.SiLU):
        super().__init__()
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        if grid_atten == True:
            self.attn = Attention(dim, dim // num_heads, grid_size=grid_size, drop=drop, grid_atten=grid_atten)
        else:
            self.attn = Attention(dim, dim // num_heads, grid_size=1, drop=drop, grid_atten=grid_atten)
...
class HATNetIR(nn.Module):
    ...
    def forward(self, x):
        x, (h_old, w_old) = self.check_image_size(x)
        ...
        return x[:, :, :h_old * self.upscale, :w_old * self.upscale]
